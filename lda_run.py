...
if __name__ == '__main__':
    ...
    args = settsrc.args; pm = settsrc.pm
    ...
    dn_op, dninfo = HyperDynamics(pm).evolve(theta_tf, L_grad_logp=grads_tf)
    tr_times = []

    with tf.Session() as sess:
        tf.global_variables_initializer().run()
        theta_smp = sess.run(dninfo.L_samples)[0]
        for i in range(args.n_round):
            t_start = time.time()
            for j in range(args.n_iter):
                tr_train_cts, tr_test_cts = data.get_batch()
                grads = model.get_grad_logp(tr_train_cts, theta=theta_smp)
                theta_smp = sess.run([dn_op, dninfo.L_samples], {grads_tf: grads})[1][0]
    ...
    resFile_root = resDir + '_'.join([pm.hyperType, pm.dnType, 'M'+str(args.M), pm.optType, pm.bwType if hasattr(pm, 'bwType') else 'void'])
    appd = -1
    while True:
