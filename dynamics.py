...
class DynamicsParams(object):
    def __init__(self, dnType, accType, optType, bwType, stepsize, **kwds):
        self.dnType = dnType
...
class DynamicsInfo():
    def __init__(self, L_samples, L_particles, L_grad_logp, L_vecfld, L_increm, global_step, stepsize, bandw, ibandw, bwmed, bwmax, var_scope):
        self.L_samples = L_samples
        self.L_particles = L_particles
        self.L_grad_logp = L_grad_logp
        self.L_vecfld = L_vecfld
        self.L_increm = L_increm
...
#
        ...
        # MCMCs. Maybe a little troublesome to fit in this framework
        elif self.pm.dnType == 'LD':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError('For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            L_vecfld = [grad_logp + tf.sqrt(2./stepsize) * tf.random_normal(particles.initialized_value().get_shape(), dtype=fltype)\
                    for grad_logp, particles in zip(L_grad_logp, L_particles)]
        elif self.pm.dnType == 'SGNHT':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError('For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            with tf.variable_scope(var_scope, reuse=reuse):
        ...
        elif self.pm.dnType == 'SGRLD-lda-expand-natural':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError('For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            L_vecfld = [tf.exp(-particles) * (grad_logp - 1.) + tf.sqrt(2./stepsize) * tf.exp(-.5*particles) * tf.random_normal(particles.initialized_value().get_shape(), dtype=fltype)\
                    for grad_logp, particles in zip(L_grad_logp, L_particles)]
        else: raise ValueError('unknown "dnType": "{}"!'.format(self.pm.dnType))
        ...
        with tf.control_dependencies([global_step] + L_vecfld):

            if self.pm.accType == 'wgd':
                L_increm = [stepsize * vecfld for vecfld in L_vecfld]
                L_op_particles = [samples.assign_add(increm) for samples, increm in zip(L_samples, L_increm)]
        ...
        return [tf.group(*L_op_particles), DynamicsInfo(L_samples, L_particles, L_grad_logp, L_vecfld, L_increm, global_step, stepsize, 1./ibandw, ibandw, bwmed, bwmax, var_scope)]
